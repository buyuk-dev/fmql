...
fmql query ./project 'type = "task"' --follow blocked_by --depth 3
fmql describe ./project   # show observed fields, types, inconsistencies
```
...
- **Pure Python**, minimal dependencies. `pip install fmql`.
...
- Optional `fmql[semantic]`, `fmql[sqlite]` extras for index backends.
